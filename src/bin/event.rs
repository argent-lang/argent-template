...
use argent_runtime::{
    EntryCall, TxBuilder, TxContext, args as entry_args, state as actor_state,
};
...
fn main() -> DemoResult<()> {
    ...
    */
    ...
    let event_before =
        actor_state! { remaining_tickets: 7, serial: 56, price: 1000 };
    let event_after =
        actor_state! { remaining_tickets: 6, serial: 57, price: 1000 };
    ...
    let ctx = TxContext::new()
        .actor_input(
            "Event",
            event_before,
            EntryCall::new("buy").args(buy_args),
            outpoint,
            utxo,
            0,
        )
